...
export const getUpcomingMovies = async (limit: number = 5) => {
  const data = await fetchMovies("/movie/upcoming");
  const limitedData = data.results.slice(0, limit);
  return limitedData;
};

export const getTrendingMovies = async (limit: number = 5) => {
  const data = await fetchMovies("/trending/movie/day");
  const limitedData = data.results.slice(0, limit);
  return limitedData;
};

export const getPopularMovies = async (limit: number = 5) => {
  const data = await fetchMovies("/movie/popular");
  const limitedData = data.results.slice(0, limit);
  return limitedData;
};

export const getTopRatedMovies = async (limit: number = 5) => {
